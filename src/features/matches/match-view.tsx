...
import { addRound, deleteRound } from "@/features/rounds/actions";
import { addNote, deleteNote, updateNote } from "@/features/notes/actions";
import { addTagToNote, removeTagFromNote } from "@/features/tags/actions";
...
export function MatchView({
  ...
  match: MatchWithRounds;
  tags: Tag[];
  userId: string;
  userName: string;
}) {
  const t = useTranslations();
  const [rounds, setRounds] = useState<Round[]>(match.rounds);
  ...
  const [noteSeverities, setNoteSeverities] = useState<
  ...
  const handleAddRound = async () => {
    const newRound = await addRound(match.id);
    setRounds([...rounds, { ...newRound, notes: [] }]);
  };
  ...
  const handleDeleteRound = async (roundId: string) => {
    ...
    try {
      await deleteRound(match.id, roundId);
    } catch {
      setRounds(prev);
    }
  };
  ...
  const handleStartEdit = (note: NoteWithTags) => {
  ...
  const handleSaveEdit = async (noteId: string) => {
    ...
    const prevState = rounds.map((r) => ({
      ...r,
      notes: r.notes.map((n) => (n.id === noteId ? { ...n } : n)),
    }));
  ...
  const handleToggleTag = async (noteId: string, tagId: string) => {
    const note = rounds
      .flatMap((r) => r.notes)
      .find((n) => n.id === noteId);
    const hasTag = note?.errorTags.some((et) => et.tag.id === tagId);
    ...
    setRounds((prev) =>
      prev.map((r) => ({
        ...
        notes: r.notes.map((n) => {
          ...
          const updatedErrorTags = hasTag
            ? n.errorTags.filter((et) => et.tag.id !== tagId)
            : (() => {
                const foundTag = tags.find((t) => t.id === tagId);
                return foundTag
                  ? [...n.errorTags, { tag: foundTag }]
                  : n.errorTags;
              })();
          return { ...n, errorTags: updatedErrorTags };
        }),
      }))
    );
  ...
  function renderNote(note: NoteWithTags, roundId: string) {
    ...
    return (
      ...
      {note.type === "ERROR" && (
        <div className="mt-1 flex flex-wrap gap-1">
          {tags.map((tag) => {
            const active = note.errorTags.some(
              (et) => et.tag.id === tag.id
            );
            return (
              <button
                key={tag.id}
                onClick={() => handleToggleTag(note.id, tag.id)}
                className={`rounded border px-1.5 py-0.5 text-xs ${
                  active
                    ? "border-cs2-orange/50 bg-cs2-orange/10 text-cs2-orange"
    ...
    <div>
      <div className="mb-6">
        <div className="flex items-start justify-between">
          ...
          <ShareLinkButton resourceType="MATCH" resourceId={match.id} />
        </div>
      </div>
      ...
      <button
        ...
        className="mt-6 w-full rounded-lg border-2 border-dashed border-white/10 py-3 text-sm text-zinc-500 hover:border-white/20 hover:text-zinc-400"
      >
        + {t("matches.round")}
      </button>
    </div>
